...
const FoldText = ({
  ...
  const segments = useMemo(() => {
    ...
    const renderSegment = (content, key, split = splitBy) => {
      segmentIndex += 1;
      return (
        <span className="fold-text-segment" data-fold-split={split} key={key}
          style={{ '--fold-perspective': `${safePerspective}px` }}>
          <span className="fold-text-piece" data-fold-hinge={hinge}
  ...
  useEffect(() => {
    ...
    let scrollTrigger, hoverHandler;
    if (trigger === 'hover') {
    ...
    return () => {
      ...
      scrollTrigger?.kill(); killTimeline();
    };
