...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Black & white by default, fades to full color on hover
          float gray = dot(color.rgb, vec3(0.299, 0.587, 0.114));
          vec3 tinted = mix(vec3(gray), color.rgb, uHover);
  ...
  update(scroll, direction, mouseWorldX) {
    ...
    // Black & white by default; lerp toward full color when the pointer is over this card.
    const halfWidth = this.plane.scale.x / 2;
    const hoverTarget =
      mouseWorldX !== null && mouseWorldX !== undefined && Math.abs(mouseWorldX - this.plane.position.x) < halfWidth
        ? 1
        : 0;
...
class App {
  constructor(
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(items, bend, textColor, borderRadius, font, cardScale, gap);
    this.update();
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font, cardScale = 1, gap = 2) {
    const defaultItems = [
  ...
  update() {
    ...
    const mouseWorldX =
      this.mouse.x !== null ? (this.mouse.x / this.screen.width - 0.5) * this.viewport.width : null;
    if (this.medias) {
      this.medias.forEach(media => media.update(this.scroll, direction, mouseWorldX));
    }
...
export default function CircularGallery({
  ...
  scrollSpeed = 2,
  scrollEase = 0.05,
  // Shrinks every card (and its label) by this factor. 1 = original React Bits size.
  cardScale = 1,
  // World-space gap between cards. Larger gap = more breathing room between labels.
  gap = 2
}) {
  ...
  }, [items, bend, textColor, borderRadius, font, fontUrl, scrollSpeed, scrollEase, cardScale, gap]);
  return (
    <div
